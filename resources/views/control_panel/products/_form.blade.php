<div class="row">
    ...
    <div class="col-sm-4">
        ...
        <div class="section">
            <div class="form-group">
                {{ Form::label('category', trans('labels.category'), ['class' => 'control-label col-sm-2']) }}
                <div class="col-sm-10">
                    <select name="product_category_id" class="form-control">
                        @foreach($categories as $category)
                            <option value="{{$category->id}}" @if(isset($product) && $product->product_category_id == $category->id) selected="selected" @endif>{{ $category->name_locale }}</option>
                            @if(isset($subcategories[$category->id]) && count($subcategories[$category->id]))
                                @foreach($subcategories[$category->id] as $subcat)
                                    <option value="{{$subcat->id}}" @if(isset($product) && $product->product_category_id == $subcat->id) selected="selected" @endif>- {{ $subcat->name_locale }}</option>
                                @endforeach
                            @endif
                        @endforeach
                    </select>
