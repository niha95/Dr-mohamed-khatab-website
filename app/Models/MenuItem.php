...
class MenuItem extends BaseModel
{
    ...
    public function getPositions()
    {
        ...
        foreach ($this->positions as $p) {
            $positions[] = trans('labels.positions.' . $p);
        }
    ...
    public function getPositionsAttribute()
    {
        ...
        if ($this->position) $positions = explode(',', $this->position);
    ...
    {
        return $this->fullUrl() == url()->current();
    }
    ...
    public function findChilds($menus)
    {
        $parent_link = $this->link;
        $childs = $menus->filter(function($item) use($parent_link){
            return starts_with($item->link,$parent_link) && $item->link != $parent_link;
        });
